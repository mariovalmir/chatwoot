class Channels::Whatsapp::WuzapiQrCodeJob < ApplicationJob
  queue_as :default
  ...
  def perform(whatsapp_channel, attempt = 1)
    ...
    if attempt > 3
      whatsapp_channel.update_provider_connection!(
        connection: 'close',
        error: 'QR code timeout - please try reconnecting'
      )
      return
    end
  ...
  def fetch_and_update_qr_code(whatsapp_channel)
    service = Whatsapp::Providers::WhatsappWuzapiService.new(whatsapp_channel: whatsapp_channel)
    ...
    Rails.logger.error "Wuzapi QR code fetch error: #{e.message}"
    whatsapp_channel.update_provider_connection!(
      connection: 'close',
      error: "Failed to fetch QR code: #{e.message}"
    )
  end
end
